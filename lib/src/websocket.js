...
`
  ...
  function reloadStyleSheet(stylePath) {
    const links = document.head.getElementsByTagName('link');
    const foundLink = Array.from(links).find(link => 
      link.rel === 'stylesheet' && (new URL(link.href)).pathname === ('/' + stylePath)
    )

    if (!foundLink) {
      console.warn('Could not find the changed style in head!', stylePath);
      return
    }

    const linkClone = foundLink.cloneNode();

    var interval = setInterval(function() {
      try {
        linkClone.sheet.cssRules; // <--- MAGIC: only populated when file is loaded
        cssLoaded();
    ...
    // only remove the old style when the new one is loaded
    function cssLoaded() {
      console.log('🔥 Css file reloaded!', stylePath);
      foundLink.remove()
    }
  ...
  var socketOnMessage = function (msg) {
    ...
    // check if cssChange message
    if (parsedData.action === "cssChange") {
      const stylePath = parsedData.message;
      console.log("CSS file changed: ", stylePath);
      reloadStyleSheet(stylePath);
      return;
    }
