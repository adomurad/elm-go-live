...
function parseUrl (pathname, model) {
  ...
  if (requestType === "static") {
      if (fs.existsSync(cleanPath)) {
          console.log(`${header} [${requestType}] ${chalk.green(cleanPath)}`);
      } else {
          console.log(`${header} [${requestType}] ${chalk.red(cleanPath)}`);
      }
  }
  else {
    console.log(`${header} [${requestType}] ${cleanPath}`);
  }
  return {
    rootPath,
    fileType,
...
function handler (model) {
  ...
  return function (req, res) {
    ...
    if (proxy && url.isType(PROXY_TYPE)) {
      ...
      return readFile(url.getPath, 'utf8')
        .alt(readFile(url.rootPath, 'utf8'))
        .fork(resolveNotFound(res, `\ngetPath (${url.getPath})\n\nand\n\nrootPath (${url.rootPath})\n\nare not found.`), resolveWith(model.reload, res))
    } else if (url.isType(RELOAD_TYPE)) {
      return readFile(RELOAD_FILE, 'utf8')
...
const setupListener = model => {
  // Removing "host: model.host" otherwise the server is not accessible
  // from other computers
  model.server.listen({ port: model.port }, listener(model))
...
/**
 * sendCssChangeMessage :: String -> Model -> Model
 * 
 * This function takes a css file path and send a message to frontend that the file is changed.
 */
function sendCssChangeMessage (message) {
  return function send (model) {
    model.server.sendMessage({ action: 'cssChange', message })
    return model
  }
}
