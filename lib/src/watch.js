...
const { sendMessage, sendCompilingMessage, sendCssChangeMessage } = require('./start')
...
function getSources_ (packageFilePath) {
  try {
    ...
    // will also parse the file and report the error - and elm-go will show it
...
const watch = model => Async((reject, _) => {
  ...
  model.log(watchingDirs(sources.concat(model.watch)))
  ...
  const cssSources = model.hotCss ? [`${model.dir}/**/*.css`] : []

  const watcher = chokidar.watch([...sources, ...packageFileNames, ...cssSources], {
    ignoreInitial: true,
  ...
  watcher.on(
    ...
    (event, filePath) => {
      ...
      if (model.useServer && model.hotCss && filePath.endsWith(".css")) {
        const relativeCssPath = path.relative(model.dir, filePath);
        if (relativeCssPath && !relativeCssPath.startsWith("..") && !path.isAbsolute(relativeCssPath)) {
          sendCssChangeMessage(relativeCssPath)(model);
        } else {
          console.warn("the changed css is not in the static base folder (--dir)");
        }
        return;
      }
